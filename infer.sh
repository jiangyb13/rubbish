...
def parse_args():
    ...
    parser.add_argument("--clip-model", type=str, default=None, help="CLIP architecture; overrides options.clip_model")
    parser.add_argument(
        "--clip-model-path", type=str,
        default="/mnt/nfs/data/pretrained_models/CLIP-ViT-H-14-laion2B-s32B-b79K/open_clip_pytorch_model.bin",
        help="Local CLIP checkpoint or OpenCLIP pretrained tag (e.g. openai)",
    )
    # Other arguments
...
def load_models(args):
    ...
    clip_arch = args.clip_model or args.options.get('clip_model', "ViT-H-14")
    clip_model, _, transform = open_clip.create_model_and_transforms(clip_arch, pretrained=args.clip_model_path, device=DEVICE)
