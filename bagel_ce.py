...
# Minimal Native CE adaptation of modeling/bagel/bagel.py.
# No CLIP, dual-view, hybrid, DropBP or in-step selector.
import copy
...
from .ce_selection import score_user_match_mismatch
...
class BagelConfig(PretrainedConfig):
    ...
    def __init__(self, visual_gen=True, visual_und=True, llm_config=None, vit_config=None, vae_config=None, latent_patch_size=2, max_latent_size=32, vit_max_num_patch_per_side=70, connector_act='gelu_pytorch_tanh', interpolate_pos=False, timestep_shift=1.0, **kwargs):
...
class Bagel(PreTrainedModel):
    ...
    def forward(self, sequence_length: int, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, sample_lens: List[int], packed_position_ids: torch.LongTensor, nested_attention_masks: List[torch.Tensor]=None, split_lens: List[int]=None, attn_modes: List[str]=None, ce_loss_indexes: Optional[torch.BoolTensor]=None, packed_label_ids: Optional[torch.LongTensor]=None, packed_vit_tokens: Optional[torch.Tensor]=None, packed_vit_token_indexes: Optional[torch.LongTensor]=None, packed_vit_position_ids: Optional[torch.LongTensor]=None, vit_token_seqlens: Optional[torch.IntTensor]=None, padded_latent: Optional[torch.Tensor]=None, patchified_vae_latent_shapes: Optional[List[Tuple[int, int]]]=None, packed_latent_position_ids: Optional[torch.LongTensor]=None, packed_vae_token_indexes: Optional[torch.LongTensor]=None, packed_timesteps: Optional[torch.LongTensor]=None, mse_loss_indexes: Optional[torch.BoolTensor]=None) -> torch.Tensor:
        ...
        if nested_attention_masks is None:
            ...
            block_mask = create_block_mask(sparse_mask, B=1, H=self.num_heads, Q_LEN=seqlen, KV_LEN=seqlen, device=packed_text_embedding.device, BLOCK_SIZE=128, _compile=True)
        ...
        if self.config.visual_und:
            ...
            packed_vit_token_embed = self.vit_model(packed_pixel_values=packed_vit_tokens, packed_flattened_position_ids=packed_vit_position_ids, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
        ...
        if self.use_moe:
            ...
            extra_inputs.update(packed_und_token_indexes=packed_und_token_indexes, packed_gen_token_indexes=packed_vae_token_indexes)
        last_hidden_state = self.language_model(packed_sequence=packed_sequence, sample_lens=sample_lens, attention_mask=attention_mask, packed_position_ids=packed_position_ids, **extra_inputs)
        mse = None
        if self.config.visual_gen:
    ...
    def forward_cache_update_text(self, past_key_values: NaiveCache, packed_text_ids: torch.IntTensor, packed_text_position_ids: torch.LongTensor, text_token_lens: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_key_value_indexes: torch.LongTensor, key_values_lens: torch.IntTensor):
        ...
        output = self.language_model.forward_inference(packed_query_sequence=packed_text_embedding, query_lens=text_token_lens, packed_query_position_ids=packed_text_position_ids, packed_query_indexes=packed_text_indexes, past_key_values=past_key_values, packed_key_value_indexes=packed_key_value_indexes, key_values_lens=key_values_lens, update_past_key_values=True, is_causal=True, **extra_inputs)
    ...
    def prepare_vit_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids):
        ...
        for image, curr_kvlen, curr_position_id in zip(images, curr_kvlens, curr_rope):
            ...
            vit_position_ids = self.get_flattened_position_ids(image_tensor.size(1), image_tensor.size(2), self.vit_patch_size, max_num_patches_per_side=self.vit_max_num_patch_per_side)
    ...
    @torch.no_grad
    def forward_cache_update_vit(self, past_key_values: NaiveCache, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_vit_tokens: torch.Tensor, packed_vit_token_indexes: torch.LongTensor, packed_vit_position_ids: torch.LongTensor, vit_token_seqlens: torch.IntTensor, packed_position_ids: torch.LongTensor, packed_seqlens: torch.IntTensor, packed_indexes: torch.LongTensor, packed_key_value_indexes: torch.LongTensor, key_values_lens: torch.IntTensor):
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
    ...
    def prepare_vae_images(self, curr_kvlens, curr_rope, images, transforms, new_token_ids, timestep=0):
        ...
        for image, curr_kvlen, curr_position_id in zip(images, curr_kvlens, curr_rope):
            ...
            vae_posiiton_ids = self.get_flattened_position_ids(image_tensor.size(1), image_tensor.size(2), self.latent_downsample, max_num_patches_per_side=self.max_latent_size)
    ...
    def forward_cache_update_vae(self, vae_model, past_key_values: NaiveCache, padded_images: torch.Tensor, patchified_vae_latent_shapes: List, packed_vae_position_ids: torch.LongTensor, packed_timesteps: torch.Tensor, packed_vae_token_indexes: torch.LongTensor, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_position_ids: torch.LongTensor, packed_seqlens: torch.IntTensor, packed_indexes: torch.LongTensor, key_values_lens: torch.IntTensor, packed_key_value_indexes: torch.Tensor):
        ...
        extra_inputs = {}
        if self.use_moe:
            extra_inputs = {'mode': 'gen', 'packed_vae_token_indexes': packed_vae_token_indexes, 'packed_text_indexes': packed_text_indexes}
        output = self.language_model.forward_inference(packed_query_sequence=packed_sequence, query_lens=packed_seqlens, packed_query_position_ids=packed_position_ids, packed_query_indexes=packed_indexes, past_key_values=past_key_values, key_values_lens=key_values_lens, packed_key_value_indexes=packed_key_value_indexes, update_past_key_values=True, is_causal=False, **extra_inputs)
        past_key_values = output.past_key_values
        return past_key_values
    ...
    def prepare_vae_latent(self, curr_kvlens, curr_rope, image_sizes, new_token_ids, model_dtype=None):
        ...
        for (H, W), curr_kvlen, curr_position_id in zip(image_sizes, curr_kvlens, curr_rope):
            ...
            if model_dtype is None:
                ...
                packed_init_noises.append(torch.randn(num_image_tokens, self.latent_channel * self.latent_patch_size ** 2).to(model_dtype))
    ...
    def generate_image(self, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_init_noises: torch.Tensor, packed_vae_position_ids: torch.LongTensor, packed_vae_token_indexes: torch.LongTensor, packed_seqlens: torch.IntTensor, packed_position_ids: torch.LongTensor, packed_indexes: torch.LongTensor, past_key_values: NaiveCache, key_values_lens: torch.IntTensor, packed_key_value_indexes: torch.LongTensor, num_timesteps: int=24, timestep_shift: float=1.0, cfg_renorm_min: float=0.0, cfg_renorm_type: str='global', cfg_interval: Optional[Tuple[float, float]]=[0, 1], cfg_text_scale: float=1.0, cfg_text_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_text_packed_position_ids: Optional[torch.LongTensor]=None, cfg_text_past_key_values: Optional[NaiveCache]=None, cfg_text_key_values_lens: Optional[torch.IntTensor]=None, cfg_text_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_img_scale: float=1.0, cfg_img_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_img_packed_position_ids: Optional[torch.LongTensor]=None, cfg_img_past_key_values: Optional[NaiveCache]=None, cfg_img_key_values_lens: Optional[torch.IntTensor]=None, cfg_img_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_type: str='parallel', enable_taylorseer=False):
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if t > cfg_interval[0] and t <= cfg_interval[1]:
                ...
                cfg_text_scale_ = 1.0
                cfg_img_scale_ = 1.0
            v_t = self._forward_flow(x_t=x_t, timestep=timestep, packed_vae_token_indexes=packed_vae_token_indexes, packed_vae_position_ids=packed_vae_position_ids, packed_text_ids=packed_text_ids, packed_text_indexes=packed_text_indexes, packed_position_ids=packed_position_ids, packed_indexes=packed_indexes, packed_seqlens=packed_seqlens, key_values_lens=key_values_lens, past_key_values=past_key_values, packed_key_value_indexes=packed_key_value_indexes, cfg_renorm_min=cfg_renorm_min, cfg_renorm_type=cfg_renorm_type, cfg_text_scale=cfg_text_scale_, cfg_text_packed_position_ids=cfg_text_packed_position_ids, cfg_text_packed_query_indexes=cfg_text_packed_query_indexes, cfg_text_key_values_lens=cfg_text_key_values_lens, cfg_text_past_key_values=cfg_text_past_key_values, cfg_text_packed_key_value_indexes=cfg_text_packed_key_value_indexes, cfg_img_scale=cfg_img_scale_, cfg_img_packed_position_ids=cfg_img_packed_position_ids, cfg_img_packed_query_indexes=cfg_img_packed_query_indexes, cfg_img_key_values_lens=cfg_img_key_values_lens, cfg_img_past_key_values=cfg_img_past_key_values, cfg_img_packed_key_value_indexes=cfg_img_packed_key_value_indexes, cfg_type=cfg_type, model_pred_cache_dic=model_pred_cache_dic, model_pred_current=model_pred_current, model_pred_text_cache_dic=model_pred_text_cache_dic, model_pred_text_current=model_pred_text_current, model_pred_img_cache_dic=model_pred_img_cache_dic, model_pred_img_current=model_pred_img_current)
    ...
    def generate_image_reca(self, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_init_noises: torch.Tensor, packed_vae_position_ids: torch.LongTensor, packed_vae_token_indexes: torch.LongTensor, packed_seqlens: torch.IntTensor, packed_position_ids: torch.LongTensor, packed_indexes: torch.LongTensor, past_key_values: NaiveCache, key_values_lens: torch.IntTensor, packed_key_value_indexes: torch.LongTensor, num_timesteps: int=24, timestep_shift: float=1.0, cfg_renorm_min: float=0.0, cfg_renorm_type: str='global', cfg_interval: Optional[Tuple[float, float]]=[0, 1], cfg_text_scale: float=1.0, cfg_text_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_text_packed_position_ids: Optional[torch.LongTensor]=None, cfg_text_past_key_values: Optional[NaiveCache]=None, cfg_text_key_values_lens: Optional[torch.IntTensor]=None, cfg_text_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_img_scale: float=1.0, cfg_img_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_img_packed_position_ids: Optional[torch.LongTensor]=None, cfg_img_past_key_values: Optional[NaiveCache]=None, cfg_img_key_values_lens: Optional[torch.IntTensor]=None, cfg_img_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_type: str='parallel', enable_taylorseer=False, clip_model=None, clip_processor=None, re_update_num=1, update_lr=[(0, 49)], use_longclip=True, use_fgclip=False, prompt=None, vae_model=None, image_shapes=None, tokenizer=None, new_token_ids=None, image_transform=None, use_save_pic=False, update_scale=100.0, model_dtype=None, save_grad=False, use_lookback=False, lookback_steps=0, use_ce_selection=False, ce_selection_prompt=None, ce_selection_vit_max_side=224, ce_selection_min_improvement=0.25, ce_selection_lookahead_steps=1, ce_selection_every_step=False):
        if use_ce_selection or use_lookback or save_grad:
            raise ValueError('This minimal CE version supports raw updates only; disable selection, lookback and save_grad.')
        self.eval()
        if enable_taylorseer:
            self.language_model.model.enable_taylorseer = True
            model_pred_cache_dic, model_pred_current = cache_init(self, num_timesteps)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            for re_update in range(re_update_num_ + 1):
                ...
                with torch.set_grad_enabled(not final_flag):
                    tt = time.time()
                    v_t = self._forward_flow_reca(x_t=x_t, timestep=timestep, packed_vae_token_indexes=packed_vae_token_indexes, packed_vae_position_ids=packed_vae_position_ids, packed_text_ids=packed_text_ids, packed_text_indexes=packed_text_indexes, packed_position_ids=packed_position_ids, packed_indexes=packed_indexes, packed_seqlens=packed_seqlens, key_values_lens=key_values_lens, past_key_values=past_key_values, packed_key_value_indexes=packed_key_value_indexes, cfg_renorm_min=cfg_renorm_min, cfg_renorm_type=cfg_renorm_type, cfg_text_scale=cfg_text_scale_, cfg_text_packed_position_ids=cfg_text_packed_position_ids, cfg_text_packed_query_indexes=cfg_text_packed_query_indexes, cfg_text_key_values_lens=cfg_text_key_values_lens, cfg_text_past_key_values=cfg_text_past_key_values, cfg_text_packed_key_value_indexes=cfg_text_packed_key_value_indexes, cfg_img_scale=cfg_img_scale_, cfg_img_packed_position_ids=cfg_img_packed_position_ids, cfg_img_packed_query_indexes=cfg_img_packed_query_indexes, cfg_img_key_values_lens=cfg_img_key_values_lens, cfg_img_past_key_values=cfg_img_past_key_values, cfg_img_packed_key_value_indexes=cfg_img_packed_key_value_indexes, cfg_type=cfg_type, model_pred_cache_dic=model_pred_cache_dic, model_pred_current=model_pred_current, model_pred_text_cache_dic=model_pred_text_cache_dic, model_pred_text_current=model_pred_text_current, model_pred_img_cache_dic=model_pred_img_cache_dic, model_pred_img_current=model_pred_img_current, final_flag=final_flag)
                    ttt = time.time()
                x_t_1 = x_t - v_t.to(x_t.device) * dts[i].cpu()
                if not final_flag:
                    x_t_0 = x_t - v_t.to(x_t.device) * t
                    loss, extra_p = self.RecALoss(prompt, x_t_0, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, extra_p=extra_p)
                    grad_x0 = torch.autograd.grad(loss, x_t_0)[0]
                    grad_x0 = grad_x0 * 1.0
                    grad = torch.autograd.grad(x_t_0, x_t, grad_outputs=grad_x0)[0]
                    grad_norm = grad.norm(p=2)
                    loss_list.append(loss.detach().item())
                    print(f'latent understanding_ce grad step {i}, update {re_update}: loss={loss.detach().item():.6g}, raw_norm={grad_norm.item():.6g}')
                    if not torch.isfinite(grad_norm):
                        raise RuntimeError(f'Non-finite CE gradient at step {i}, update {re_update}')
                    if grad_norm > 0.01:
                        grad = grad * (0.01 / grad_norm)
        ...
        unpacked_latent = x_t.split((packed_seqlens - 2).tolist())
        en = time.time()
        print(f'total time: {en - st}')
        return unpacked_latent

    def _forward_flow_reca(self, x_t: torch.Tensor, timestep: torch.LongTensor, packed_vae_token_indexes: torch.LongTensor, packed_vae_position_ids: torch.LongTensor, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_indexes: torch.LongTensor, packed_position_ids: torch.LongTensor, packed_seqlens: torch.IntTensor, key_values_lens: torch.IntTensor, past_key_values: NaiveCache, packed_key_value_indexes: torch.LongTensor, cfg_renorm_min: float=0.0, cfg_renorm_type: str='global', cfg_text_scale: float=1.0, cfg_text_packed_position_ids: Optional[torch.LongTensor]=None, cfg_text_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_text_key_values_lens: Optional[torch.Tensor]=None, cfg_text_past_key_values: Optional[NaiveCache]=None, cfg_text_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_img_scale: float=1.0, cfg_img_packed_position_ids: Optional[torch.LongTensor]=None, cfg_img_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_img_key_values_lens: Optional[torch.Tensor]=None, cfg_img_past_key_values: Optional[NaiveCache]=None, cfg_img_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_type: str='parallel', model_pred_cache_dic: Optional[Dict[str, Any]]=None, model_pred_current: Optional[int]=None, model_pred_text_cache_dic: Optional[Dict[str, Any]]=None, model_pred_text_current: Optional[int]=None, model_pred_img_cache_dic: Optional[Dict[str, Any]]=None, model_pred_img_current: Optional[int]=None, final_flag=False):
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
        packed_sequence = packed_text_embedding.new_zeros((sum(packed_seqlens), self.hidden_size))
        packed_sequence[packed_text_indexes] = packed_text_embedding
    ...
    @torch.no_grad
    def _forward_flow(self, x_t: torch.Tensor, timestep: torch.LongTensor, packed_vae_token_indexes: torch.LongTensor, packed_vae_position_ids: torch.LongTensor, packed_text_ids: torch.LongTensor, packed_text_indexes: torch.LongTensor, packed_indexes: torch.LongTensor, packed_position_ids: torch.LongTensor, packed_seqlens: torch.IntTensor, key_values_lens: torch.IntTensor, past_key_values: NaiveCache, packed_key_value_indexes: torch.LongTensor, cfg_renorm_min: float=0.0, cfg_renorm_type: str='global', cfg_text_scale: float=1.0, cfg_text_packed_position_ids: Optional[torch.LongTensor]=None, cfg_text_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_text_key_values_lens: Optional[torch.Tensor]=None, cfg_text_past_key_values: Optional[NaiveCache]=None, cfg_text_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_img_scale: float=1.0, cfg_img_packed_position_ids: Optional[torch.LongTensor]=None, cfg_img_packed_query_indexes: Optional[torch.LongTensor]=None, cfg_img_key_values_lens: Optional[torch.Tensor]=None, cfg_img_past_key_values: Optional[NaiveCache]=None, cfg_img_packed_key_value_indexes: Optional[torch.LongTensor]=None, cfg_type: str='parallel', model_pred_cache_dic: Optional[Dict[str, Any]]=None, model_pred_current: Optional[int]=None, model_pred_text_cache_dic: Optional[Dict[str, Any]]=None, model_pred_text_current: Optional[int]=None, model_pred_img_cache_dic: Optional[Dict[str, Any]]=None, model_pred_img_current: Optional[int]=None):
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
        packed_sequence = packed_text_embedding.new_zeros((sum(packed_seqlens), self.hidden_size))
        packed_sequence[packed_text_indexes] = packed_text_embedding
        assert timestep.unique().shape[0] == 1
    ...
    def prepare_start_tokens(self, curr_kvlens, curr_rope, new_token_ids):
        ...
        for curr_kvlen, curr_position_id in zip(curr_kvlens, curr_rope):
            ...
            curr += curr_kvlen
        generation_input = {'packed_start_tokens': torch.tensor(packed_start_tokens, dtype=torch.long), 'packed_query_position_ids': torch.tensor(packed_query_position_ids, dtype=torch.long), 'key_values_lens': torch.tensor(curr_kvlens, dtype=torch.int), 'packed_key_value_indexes': torch.tensor(packed_key_value_indexes, dtype=torch.long)}
        return generation_input

    @torch.no_grad
    def generate_text(self, past_key_values: NaiveCache, packed_key_value_indexes: torch.LongTensor, key_values_lens: torch.IntTensor, packed_start_tokens: torch.LongTensor, packed_query_position_ids: torch.LongTensor, max_length: int, do_sample: bool=False, temperature: float=1.0, end_token_id: int=None):
        step = 0
        generated_sequence = []
    ...
    @torch.no_grad()
    def chat(self, tokenizer, new_token_ids, image_transform, images, prompt, max_length: int, do_sample: bool=False, temperature: float=1.0):
        ...
        new_rope = [0]
        for image in images:
            generation_input, newlens, new_rope = self.prepare_vit_images(curr_kvlens=newlens, curr_rope=new_rope, images=[image], transforms=image_transform, new_token_ids=new_token_ids)
            for k, v in generation_input.items():
                if torch.is_tensor(v):
                    generation_input[k] = v.to(device)
            with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                past_key_values = self.forward_cache_update_vit(past_key_values, **generation_input)
        generation_input, newlens, new_rope = self.prepare_prompts(curr_kvlens=newlens, curr_rope=new_rope, prompts=[prompt], tokenizer=tokenizer, new_token_ids=new_token_ids)
        for k, v in generation_input.items():
            if torch.is_tensor(v):
                generation_input[k] = v.to(device)
        with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
        ...
        for k, v in generation_input.items():
            if torch.is_tensor(v):
                generation_input[k] = v.to(device)
        with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
            unpacked_latent = self.generate_text(past_key_values=past_key_values, max_length=max_length, do_sample=do_sample, temperature=temperature, end_token_id=new_token_ids['eos_token_id'], **generation_input)
    ...
    def _ce_cache_update_text(self, past_key_values, text, curr_kvlens, curr_rope, tokenizer, new_token_ids, device):
        generation_input, newlens, new_rope = self.prepare_prompts(curr_kvlens=curr_kvlens, curr_rope=curr_rope, prompts=[text], tokenizer=tokenizer, new_token_ids=new_token_ids)
        for k, v in generation_input.items():
            if torch.is_tensor(v):
                generation_input[k] = v.to(device)
        ...
        output = self.language_model.forward_inference(packed_query_sequence=packed_text_embedding, query_lens=generation_input['text_token_lens'], packed_query_position_ids=generation_input['packed_text_position_ids'], packed_query_indexes=generation_input['packed_text_indexes'], past_key_values=past_key_values, packed_key_value_indexes=generation_input['packed_key_value_indexes'], key_values_lens=generation_input['key_values_lens'], update_past_key_values=True, is_causal=True, **extra_inputs)
        return (output.past_key_values, newlens, new_rope)

    def _prepare_understanding_ce_visual_context(self, image, image_transform, new_token_ids, ce_vit_max_side):
        ...
        if max(image_h, image_w) > ce_vit_max_side:
            ...
            resized_h = max(self.vit_patch_size, round(image_h * scale / self.vit_patch_size) * self.vit_patch_size)
            resized_w = max(self.vit_patch_size, round(image_w * scale / self.vit_patch_size) * self.vit_patch_size)
            image_tensor = F.interpolate(image_tensor.unsqueeze(0), size=(resized_h, resized_w), mode='bicubic', align_corners=False, antialias=False)[0]
        mean = torch.as_tensor(image_transform.normalize_transform.mean, device=image_tensor.device, dtype=image_tensor.dtype)[:, None, None]
        std = torch.as_tensor(image_transform.normalize_transform.std, device=image_tensor.device, dtype=image_tensor.dtype)[:, None, None]
        image_tensor = (image_tensor - mean) / std
        vit_position_ids = self.get_flattened_position_ids(image_tensor.shape[-2], image_tensor.shape[-1], self.vit_patch_size, max_num_patches_per_side=self.vit_max_num_patch_per_side).to(device)
        packed_vit_tokens = patchify(image_tensor, self.vit_patch_size).to(device)
        num_vit_tokens = packed_vit_tokens.shape[0]
        print(f'ce vit input: {image_tensor.shape[-2]}x{image_tensor.shape[-1]}, tokens: {num_vit_tokens}')
        vit_token_seqlens = torch.tensor([num_vit_tokens], dtype=torch.int, device=device)
        cu_seqlens = F.pad(torch.cumsum(vit_token_seqlens, dim=0), (1, 0)).to(torch.int32)
        packed_vit_embed = self.vit_model(packed_pixel_values=packed_vit_tokens, packed_flattened_position_ids=vit_position_ids, cu_seqlens=cu_seqlens, max_seqlen=num_vit_tokens)
        packed_vit_embed = self.connector(packed_vit_embed)
        packed_vit_embed = packed_vit_embed + self.vit_pos_embed(vit_position_ids)
        image_boundary_ids = torch.tensor([token_id('start_of_image'), token_id('end_of_image')], dtype=torch.long, device=device)
        image_boundary_embed = self.language_model.model.embed_tokens(image_boundary_ids)
        packed_sequence = image_boundary_embed.new_zeros((num_vit_tokens + 2, self.hidden_size))
        ...
        image_seqlens = torch.tensor([num_vit_tokens + 2], dtype=torch.int, device=device)
        image_indexes = torch.arange(num_vit_tokens + 2, dtype=torch.long, device=device)
        image_position_ids = torch.zeros(num_vit_tokens + 2, dtype=torch.long, device=device)
        empty_kv_indexes = torch.empty(0, dtype=torch.long, device=device)
        empty_kv_lens = torch.zeros(1, dtype=torch.int, device=device)
        extra_inputs = {'mode': 'und'} if self.use_moe else {}
        image_output = self.language_model.forward_inference(packed_query_sequence=packed_sequence, query_lens=image_seqlens, packed_query_position_ids=image_position_ids, packed_query_indexes=image_indexes, past_key_values=ce_cache, key_values_lens=empty_kv_lens, packed_key_value_indexes=empty_kv_indexes, update_past_key_values=True, is_causal=False, **extra_inputs)
        return (image_output.past_key_values, [num_vit_tokens + 2], [1])

    def UnderstandingCELoss(self, x_t_0, target_text, vae_model, image_shape, image_transform, tokenizer, new_token_ids, ce_max_tokens=192, ce_vit_max_side=168, instruction=None):
        ...
        def token_id(name):
            value = new_token_ids[name]
            return int(value.item()) if torch.is_tensor(value) else int(value)
        image = self._decode_reca_latent_to_tensor(x_t_0, image_shape, vae_model)
        prepared_visual_context = self._prepare_understanding_ce_visual_context(image, image_transform, new_token_ids, ce_vit_max_side)
        ...
        if instruction is None:
            instruction = 'Describe this image in one concise paragraph with concrete visual details and atmosphere. Only one paragraph is required.'
        ce_cache, curr_kvlens, curr_rope = self._ce_cache_update_text(ce_cache, instruction, curr_kvlens, curr_rope, tokenizer, new_token_ids, device)
        ...
        output = self.language_model.forward_inference(packed_query_sequence=packed_text_embedding, query_lens=query_lens, packed_query_position_ids=packed_query_position_ids, packed_query_indexes=packed_query_indexes, past_key_values=ce_cache, key_values_lens=key_values_lens, packed_key_value_indexes=packed_key_value_indexes, update_past_key_values=False, is_causal=True, **extra_inputs)
    ...
    def RecALoss(self, prompt, img_latent, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, clip_model=None, clip_processor=None, use_longclip=False, use_fgclip=False, extra_p=None):
        """Reflect once, then score a fixed target with native image-caption CE.

    img_latent is the current look-ahead x0 estimate, not the noisy xt.
    CLIP arguments remain only for legacy call compatibility; never used.
    """
        if isinstance(img_latent, (list, tuple)):
            if len(img_latent) != 1:
                raise ValueError('Native CE expects one image per call')
            img_latent = img_latent[0]
        if extra_p is None:
            draft = self._decode_reca_latent_to_pil(img_latent.detach(), image_shapes, vae_model)
            instruction = f"User request: '{prompt}'. Look at the draft and consider what needs to change to fulfill the request. Then describe the image after those changes in one or two concise sentences. Bind each requested property to its object and make unclear requested details recognizable. Preserve compatible visible details. Output only the resulting image description, about 30 to 50 words."
            extra_p = self.chat(tokenizer=tokenizer, new_token_ids=new_token_ids, image_transform=image_transform, images=[draft], prompt=instruction, max_length=112, do_sample=False)
            if '键' in extra_p:
                extra_p = prompt
