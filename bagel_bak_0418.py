...
class Bagel(PreTrainedModel):
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            for re_update in range(re_update_num_ + 1):
                ...
                if not final_flag:
                    ...
                    # breakpoint()
                    t1 = time.time()
                    loss, vlm_output_text = self.RecALoss(prompt, x_t_0_latent, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, clip_model, clip_processor, use_longclip, use_fgclip, extra_p=extra_p)
                    extra_p = vlm_output_text
                    # print(f"vlm_out:{vlm_output_text}")
    ...
    def RecALoss(self, prompt, img_latent, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, clip_model, clip_processor, use_longclip, use_fgclip, extra_p):
        def decode_image(latent, image_shape, vae_model):
                H, W = image_shape
                h, w = H // self.latent_downsample, W // self.latent_downsample
                latent = latent.reshape(1, h, w, self.latent_patch_size, self.latent_patch_size, self.latent_channel)
        ...
        if extra_p is None:
            ...
            vlm_output_text=prompt
        # vlm_output_text = prompt
        # vlm_output_text = vlm_output_text.replace("键","")
        # breakpoint()
    ...
    def chat_only(self, prompt, img_latent, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, clip_model, clip_processor, use_longclip, use_fgclip):
        ...
        vlm_output_text = self.chat(tokenizer=tokenizer,
            ...
            prompt=f"""
            Act as a strict AI Art Critic and Quality Assurance Specialist. 
            I will provide you with a generated image and the original text prompt used to create it.

            User Prompt: '{prompt}'

            Your task is to evaluate the image and assign a quality score from 0 to 10.

            ### Scoring Rubric:
            - **10 (Perfect):** Flawless execution. Perfectly follows the prompt. Artistic masterpiece. No artifacts.
            - **8-9 (Excellent):** Highly detailed and accurate. Minor, negligible inconsistencies.
            - **6-7 (Passable):** Follows the main idea but has visible artifacts (e.g., bad hands, blurry background) or misses minor details.
            - **4-5 (Poor):** Misses key elements of the prompt OR has significant distortions/glitches.
            - **0-3 (Failure):** Completely unrelated to the prompt, severely corrupted image, or nightmare fuel.

            ### Instructions:
            1. First, analyze the image for **Negative Constraints**: Look for extra fingers, distorted limbs, blurred faces, or object bleeding.
            2. Second, analyze **Prompt Adherence**: Did it draw exactly what was asked?
            3. Finally, assign an integer score strictly based on the rubric above.

            ### Output Format:
            Return ONLY a raw JSON object (no markdown, no code blocks):
            {{"reasoning": "Brief explanation of flaws or merits...", "score": <integer_0_to_10>}}
            """,
            # prompt=f"Based on the original prompt “{prompt}”, analyze the intermediate image for flaws in color harmony, object/ID accuracy, and spatial/positional relationships.Now, generate a single, concise paragraph (under 200 words) describing the ideal, perfected target image. Do not describe the current flaws; instead, paint a vivid, coherent vision of the final, refined result that perfectly fulfills the prompt's intent.",
            max_length=192)
        import re
        import json
        match = re.search(r"\{.*\}", vlm_output_text, re.DOTALL)
