...
class Bagel(PreTrainedModel):
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            for re_update in range(re_update_num_ + 1):
                ...
                if not final_flag:
                    ...
                    t3 = time.time()
                    # print(f"loss calc time: {t2 - t1}, grad calc time: {t3 - t2}")
                    # breakpoint()
    ...
    def RecALoss(self, prompt, img_latent, vae_model, image_shapes, tokenizer, new_token_ids, image_transform, clip_model, clip_processor, use_longclip, use_fgclip, extra_p):
        ...
        if extra_p is None:
            with torch.no_grad():
                vlm_output_text = self.chat(tokenizer=tokenizer,
                    ...
                    prompt=f"Based on the text prompt '{prompt}' and the provided noisy intermediate image, describe the envisioned perfect final image in one concise sentence. Ignore noise and artifacts, focusing strictly on the visual details and atmosphere. If there is any conflict between the visual features and the text prompt, you must strictly follow the text prompt. Only one sentence is required.",
                    max_length=60)
        else :
            vlm_output_text=prompt
        ...
        if use_longclip:
            ...
            flag = False
            if self.self_prompt is None or self.self_prompt != vlm_output_text:
                flag = True

            if flag:
                text_inputs = longclip.tokenize([vlm_output_text]).to("cuda")
                text_embeds = clip_model.encode_text(text_inputs)
                self.prompt_clip_feature = text_embeds
